function solve(input = []){
    ...
    for(let i = 0; i < input.length; i++){
        ...
        switch(command){
            case `Join`: join(name); break;
            case `Jump`: jump(name, index); break;
            case `Dive`: dive(subCommand); break;
            case `First`: first(subCommand); break;
            case `Last`: last(subCommand); break;
            case `Print`: print(subCommand); break;
        }
    }
    ...
    function last(subCommand){
        if(subCommand > squad.length){
            ...
            console.log(squad.slice(Math.max(squad.length - subCommand, 0)).join(` `));
        }
    }
